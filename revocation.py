...
class Revocation:
    ...
    def GetOldMsg(self, msg):
        ...
        print u"进入了GetOldMsg"
        print msg['Content']
        print re.search(ur"\!\[CDATA\[.*撤回了一条消息\]\]", msg['Content'])
        if re.search(ur"\!\[CDATA\[.*撤回了一条消息\]\]", msg['Content']):
    ...
    def GetMsgFrom(self, msg):
        ...
        if itchat.search_chatrooms(userName=msg['FromUserName']):
            ...
            msg_group += u"[%s] " % itchat.search_chatrooms(userName=msg['FromUserName'])['NickName']
    ...
    def GetMsgToSend(self, old_msg, msg_time_to_user):
        ...
        msg_send += u"时间: %s\n谁: %s\n" % (msg_time_to_user, old_msg.get('msg_from', None))

        if old_msg.get('msg_group', None):
            msg_send += u"群组: %s\n" % old_msg['msg_group']

        msg_send += u"类型: %s\n内容: %s\n" % (old_msg.get('msg_type', None), old_msg.get('msg_content', None))

        if old_msg['msg_type'] == "Sharing":
            msg_send += u"网址: %s\n" % old_msg.get('msg_url', None)

        elif old_msg['msg_type'] in ['Picture', 'Recording', 'Video', 'Attachment']:
            msg_send += u"存储: Revocation文件夹中\n使用命令: 查看文件[%s]" % old_msg.get('msg_content', None)
